...
?>
           ...
           <?php
            ...
            $query1 = $this->db->query("SELECT SUM(uang_muka) as total_non_cash_tahap1 FROM transaksi WHERE pembayaran IN ('edc', 'transfer', 'qris') AND tanggal = '".$tanggal1."'");
            $result1 = $query1->row();
            $total_non_cash_tahap1 = $result1->total_non_cash_tahap1;

            // Lakukan query untuk pembayaran non-cash pada tahap kedua (pengambilan)
            $query2 = $this->db->query("SELECT SUM(sisa) as total_non_cash_tahap2 FROM transaksi WHERE pembayaran_sisa IN ('edc', 'transfer', 'qris') AND tanggal = '".$tanggal1."'");
            $result2 = $query2->row();
...
<!-- $this->db->select('transaksi.*, stock_lensa.*');
$this->db->from('transaksi');
$this->db->join('stock_lensa', 'transaksi.lensa = stock_lensa.id');
$this->db->where('stock_lensa.jenis_lensa', 'kriptok'); // Perhatikan penulisan "kriptok" di sini
$this->db->where('transaksi.bpjs', 165000);
...
foreach ($transaksi as $value) {
    // Lakukan apa yang perlu dilakukan dengan setiap baris transaksi di sini
    // Misalnya, tampilkan nilai bpjs
    echo $value->bpjs;
